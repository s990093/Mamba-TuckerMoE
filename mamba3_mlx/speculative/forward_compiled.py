...
def make_compiled_model_verify_forward(model) -> Callable:
    """Build a verify-forward function with Mamba layers JIT-fused.

    Drop-in replacement for :func:`mamba3_mlx.speculative.forward.model_verify_forward`.
    Compilation work happens lazily on first call (first cold + first warm
    invocation per layer); subsequent calls reuse the cached graphs.

    Transformer layers remain uncompiled and pay the usual MLX dispatch cost.
    Use the regular ``model_verify_forward`` from ``.forward`` if you want
    100% identical execution path including the Transformer layers.

    Returns
    -------
    fn(model, input_ids, states=None) -> (logits, payload)
        Signature-identical to :func:`model_verify_forward`.  The ``model``
        argument is accepted for API parity but is ignored — the model the
        compiled steps capture is the one passed here at build time.
    """
