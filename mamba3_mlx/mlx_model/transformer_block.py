...
class TransformerBlock(nn.Module):
    ...
    def __call__(self, x, state=None):
        ...
        # Expand kv groups for GQA
        if self.kv_groups > 1:
            k_exp = mx.repeat(k, self.kv_groups, axis=1)
            v_exp = mx.repeat(v, self.kv_groups, axis=1)
        else:
            k_exp = k
            v_exp = v
        ...
        if L == 1:
            attn = mx.fast.scaled_dot_product_attention(q, k_exp, v_exp, scale=self.scale)
        else:
