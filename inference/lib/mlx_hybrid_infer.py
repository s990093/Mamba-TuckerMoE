...
_ultimate_kernels = UltimateMambaKernels()
...
class Mamba3Config:
    def __init__(
        ...
        #: AMX simdgroup_matrix partial-output kernel (decode b=1 only; fastest benchmarked path).
        self.tucker_amx_fuse: bool = False
        #: Scalar optimized kernel for decode b=1.
        self.tucker_scalar_fuse: bool = False
        #: Fused Mamba Mixer: Norm+RoPE+Einsum1+Lambda+SSM+Einsum2 in one Metal dispatch.
        self.fused_mamba_mixer: bool = False
...
class TuckerMoE(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        # AMX simdgroup_matrix path — fastest for single-token decode (b=1, bfloat16 only).
        do_amx = bool(amx_partial_fuse) if amx_partial_fuse is not None else False
        do_scalar = bool(scalar_fuse) if scalar_fuse is not None else False
        num_experts, r3, r2 = g_all.shape
        
        if do_scalar and x_flat.shape[0] == 1:
            # Batch=1 optimized scalar kernel
            scalar_fn = _ultimate_kernels.tucker.build_scalar(r3, r2, num_experts, self.top_k)
            xi = x_shared[0]
            idx = top_k_indices[0].astype(mx.uint32)
            partial = _ultimate_kernels.tucker.run_scalar(
                scalar_fn, xi, g_all, idx, r2, dtype=x_flat.dtype
            ) # Note: scalar kernel already sums over K!
            out = self.U_out(partial[None, :]).reshape(*orig_shape[:-1], -1)
            return out + self.bias
            
        if (
            do_amx
            and x_flat.shape[0] == 1
            and x_flat.dtype == mx.bfloat16
            and r3 % 32 == 0
            and r2 % 32 == 0
        ):
            amx_fn = _ultimate_kernels.tucker.build_amx_async(r3, r2, num_experts, self.top_k)
            xi = x_shared[0]  # (r3,)
            idx = top_k_indices[0].astype(mx.uint32)  # (k,)
            partial = _ultimate_kernels.tucker.run_amx_async(
                amx_fn, xi, g_all, idx, r2, self.top_k, dtype=mx.bfloat16
            ) # (k, r2)
            probs_k = top_k_probs[0]  # (k,)
            out_core = mx.sum(partial * probs_k[:, None], axis=0, keepdims=True)  # (1, r2)
...
class MixtralMoEFeedForward(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        gate = self.gate_proj(x, router_temp, router_x=anch_up, einsum_fuse=einsum_fuse, full_fuse=full_fuse, amx_partial_fuse=amx_partial_fuse, scalar_fuse=scalar_fuse)
        feat = self.up_proj(x, router_temp, router_x=anch_up, einsum_fuse=einsum_fuse, full_fuse=full_fuse, amx_partial_fuse=amx_partial_fuse, scalar_fuse=scalar_fuse)
        return self.down_proj(
            ...
            full_fuse=full_fuse,
            amx_partial_fuse=amx_partial_fuse,
            scalar_fuse=scalar_fuse,
        )
...
class Mamba3Block(nn.Module):
    def __init__(self, config: Mamba3Config):
        ...
        self._fused_mixer_kernel: Optional[Any] = None
    ...
    def __call__(
        ...
        _af = getattr(self.config, "tucker_amx_fuse", False)
        xp_dim = int(h * p)
        ...
        if self.config.use_kmoe:
            x_up = self.x_up_proj(
                ...
                einsum_fuse=_ef,
                full_fuse=_ff,
                amx_partial_fuse=_af,
                scalar_fuse=getattr(self.config, "tucker_scalar_fuse", False),
            )
            x_ssm = x_up.reshape(b_sz, l, h, p, r)
        else:
            x_ssm = self.x_up_proj(x_prime).reshape(b_sz, l, h, p, r)

        # ── Fused Mamba Mixer: single Metal dispatch for decode (batch=1, seq=1) ──
        _use_fused = (
            getattr(self.config, "fused_mamba_mixer", False)
        ...
        if _use_fused:
            ...
            lv_flat = mx.sigmoid(
                self._bg(mx.expand_dims(lambda_param, -1)).squeeze(-1)
            ).reshape(-1)

            if self._fused_mixer_kernel is None:
                self._fused_mixer_kernel = _ultimate_kernels.mamba_mixer.build(h, n, p, r)

            h_final_k, inp_sig_k, y_k, new_ang_k = _ultimate_kernels.mamba_mixer.run(
                self._fused_mixer_kernel,
                b_raw=b_param.reshape(-1),
                c_raw=c_param.reshape(-1),
                norm_b_w=self.norm_B.weight,
                norm_c_w=self.norm_C.weight,
                bias_b=mx.flatten(self.bias_B),
                bias_c=mx.flatten(self.bias_C),
                theta_rep=theta_rep,
                prev_angle=prev_angle_sum.reshape(h, n // 2),
                x_ssm=x_ssm.reshape(h, p, r),
                prev_h=prev_h.reshape(h, n, p),
                prev_input=prev_input.reshape(h, n, p),
                dt_b=dt_b.reshape(-1),
                a_b=a_b.reshape(-1),
                lv=lv_flat,
                h=h, n=n, p=p, r=r,
            ...
            h_final = h_final_k.reshape(b_sz, h, n, p)
            y_stack = y_k.reshape(b_sz, l, h, p, r)
            new_cache = (
                h_final,
                inp_sig_k.reshape(b_sz, 1, h, n, p),
                new_ang_k.reshape(b_sz, 1, h, n // 2),
            )
        else:
    ...
    def __call__(
        ...
        if self.use_kmoe:
            ffn_out = self.ffn(
                ...
                amx_partial_fuse=getattr(self.config, "tucker_amx_fuse", False),
                scalar_fuse=getattr(self.config, "tucker_scalar_fuse", False),
            )
        else:
    ...
    if not ck:
        ...
        if os.path.isdir(ckpt_dir):
            for f in sorted(os.listdir(ckpt_dir)):
                if f.endswith(".npz") and not force_pt:
                    return (os.path.join(ckpt_dir, f), "npz")
                if f.endswith(".pt"):
                    p = os.path.join(ckpt_dir, f)
                    side = _try_npz_for_pt(p)
                    if side is not None:
                        return (side, "npz")
                    return (p, "pt")
        return (None, "none")
