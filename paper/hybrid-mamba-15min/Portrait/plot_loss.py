...
FORMULAS = [
    ...
    r"$\mathcal{L}_{\mathrm{total}}\approx 92\%\mathrm{CE}+8\%\mathrm{FCP}+\mathrm{aux}$",
]
...
COT_STEPS = stage_max_step(COT_SFT_TRAIN_CSV)

pre_token_start = 0
ins_token_start = PRE_STEPS * PRE_TOKENS_PER_STEP
cot_token_start = ins_token_start + INS_STEPS * SFT_TOKENS_PER_STEP
boundaries = [
    pre_token_start,
    ins_token_start,
    cot_token_start,
    cot_token_start + COT_STEPS * SFT_TOKENS_PER_STEP,
]
...
cot_train_x, cot_train_loss = load_stage(COT_SFT_TRAIN_CSV, cot_token_start, SFT_TOKENS_PER_STEP)
...
cot_val_x, cot_val_loss = load_stage(COT_SFT_VAL_CSV, cot_token_start, SFT_TOKENS_PER_STEP)
...
print(
    ...
    f"cot={boundaries[3]-boundaries[2]:,} total={boundaries[-1]:,}"
)
